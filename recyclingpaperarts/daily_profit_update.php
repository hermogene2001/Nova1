...
try {
    ...
    while ($purchase = $result->fetch_assoc()) {
        ...
        $checkQuery = "SELECT COUNT(*) FROM transactions WHERE client_id = ? AND transaction_type = 'daily_earning' AND date = ?";
        ...
        if ($now < $endDatetime) {
            ...
            $transactionQuery = "INSERT INTO transactions (client_id, transaction_type, amount, date) VALUES (?, 'daily_earning', ?, ?)";
    ...
    while ($expired = $expiredResult->fetch_assoc()) {
        ...
        $capitalTransactionQuery = "INSERT INTO transactions (client_id, transaction_type, amount, date) VALUES (?, 'capital_return', ?, ?)";
