...
<?php 
        // Fetch support phone number from database
        $phoneSql = "SELECT support_phone FROM social_links WHERE id = 1 LIMIT 1";
        $phoneStmt = $conn->prepare($phoneSql);
        $phoneStmt->execute();
        $phoneStmt->bind_result($supportPhone);
        $phoneStmt->fetch();
        $phoneStmt->close();
        
        if (!empty($supportPhone)): ?>
            <a href="tel:<?php echo htmlspecialchars($supportPhone); ?>" target="_blank" aria-label="Call <?php echo htmlspecialchars($supportPhone); ?>">
                <i class="fas fa-phone" style="color:rgb(34, 87, 201);"></i>
                CallUs On <?php echo htmlspecialchars($supportPhone); ?>
            </a>
        <?php endif; ?>
